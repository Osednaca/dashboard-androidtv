...
class BusinessQuickPlayTest extends TestCase
{
    ...
    {
        return array_replace(['media_asset_id' => $media->id, 'display_mode' => 'business', 'scope' => 'all', 'duration' => 10], $patch);
    }

    public function test_options_and_all_scope_only_include_the_current_business(): void
    {
        ...
        $this->actingAs($this->businessUser($business))->get('/business/quick-play/create')
            ...
            ->has('options.businesses', 1)->has('options.locations', 1)
    ...
    public function test_own_device_location_and_business_scopes_can_send_fullscreen(): void
    {
        [$business, $device, $media, $location] = $this->fixture();
        $this->actingAs($this->businessUser($business));
        foreach (['devices' => ['device_ids' => [$device->id]], 'locations' => ['location_ids' => [$location->id]], 'businesses' => ['business_ids' => [$business->id]]] as $scope => $ids) {
            $this->post('/business/quick-play', $this->payload($media, $ids + ['scope' => $scope, 'display_mode' => 'fullscreen']))
                ->assertSessionHasNoErrors()->assertRedirect();
        }
        $this->assertDatabaseCount('device_commands', 3);
        $this->assertDatabaseHas('quick_plays', ['business_id' => $business->id, 'display_mode' => 'fullscreen']);
    }
    ...
    public function test_empty_targets_missing_duration_and_unready_media_do_not_send(): void
    {
        [$business, , $media] = $this->fixture();
        $this->actingAs($this->businessUser($business));
        $this->post('/business/quick-play', $this->payload($media, ['scope' => 'devices']))->assertSessionHasErrors('targets');
